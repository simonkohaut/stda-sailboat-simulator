...
param_dict = yaml.load(sim_params, Loader=Loader)
...
# Hydrostatic force
HYDROSTATIC_EFF_X = HEIGHT_BOUYANCY + \
    (WATER_DENSITY / MASS) * GEOMETRICAL_MOI_X
HYDROSTATIC_EFF_Y = HEIGHT_BOUYANCY + \
    (WATER_DENSITY / MASS) * GEOMETRICAL_MOI_Y
...
DAMPING_INVARIANT_Z = -.5 * DAMPING_Z * \
    sqrt(WATER_DENSITY * WATERLINE_AREA * GRAVITY * MASS)
DAMPING_INVARIANT_YAW = -(MOI_Z / YAW_TIMECONSTANT)
DAMPING_INVARIANT_PITCH = -2 * PITCH_DAMPING * \
    sqrt(MOI_Y * MASS * GRAVITY * HYDROSTATIC_EFF_Y)
DAMPING_INVARIANT_ROLL = -2 * ROLL_DAMPING * \
    sqrt(MOI_X * MASS * GRAVITY * HYDROSTATIC_EFF_X)
...
POS_X,     POS_Y,      POS_Z,             \
    ROLL,      PITCH,      YAW,               \
    VEL_X,     VEL_Y,      VEL_Z,             \
    ROLL_RATE, PITCH_RATE, YAW_RATE = list(range(12))
...
SAIL_ANGLE, RUDDER_ANGLE, TRUE_WIND, WAVE = list(range(4))
...
def calculate_sail_force(roll, wind, sail_angle):
    ...
    pressure = (AIR_DENSITY / 2) * wind.speed**2 * \
        cos(roll * cos(sail_angle))**2
    ...
    separated_propulsion = sign(aoa) * pressure * \
        SAIL_AREA * sin(aoa)**2 * sin(sail_angle)
    separated_transverse_force = - \
        sign(aoa) * pressure * SAIL_AREA * sin(aoa)**2 * cos(sail_angle)
...
def calculate_lateral_force(vel_x, vel_y, roll, speed):
    ...
    separated_transverse_force = - \
        sign(aoa) * pressure * SAIL_AREA * sin(aoa)**2

    return LateralForce(
        x=(1 - separation) * (tmp * cos(aoa) + 2 * pi *
                              eff_aoa * sin(aoa)) * pressure * LATERAL_AREA,
        y=(1 - separation) * (tmp * sin(aoa) - 2 * pi * eff_aoa * cos(aoa)) * pressure * LATERAL_AREA + separation * separated_transverse_force), separation
...
def calculate_rudder_force(speed, rudder_angle):
    ...
    return RudderForce(
        x=-(((4 * pi) / RUDDER_STRETCHING) * rudder_angle**2) *
        pressure * RUDDER_BLADE_AREA,
...
def calculate_hydrostatic_force(pos_z, roll, pitch, wave_influence):
    ...
    '''
    force = HYDROSTATIC_INVARIANT_Z * \
        (pos_z - wave_influence.height) + GRAVITY_FORCE
...
def solve(time, boat):
    ...
    damping = calculate_damping(
        vel_x, vel_y, vel_z, roll_rate, pitch_rate, yaw_rate)
    hydrostatic_force, x_hs, y_hs = calculate_hydrostatic_force(
        pos_z, roll, pitch, wave_influence)
    ...
    delta_yaw_rate = (damping.yaw
                      ...
                      + sail_force.x * sin(true_sail_angle) * \
                      DISTANCE_MAST_SAIL_PRESSURE_POINT
    ...
    if actor_dynamics:
        ...
        delta_rudder = np.clip(
            delta_rudder, -max_rudder_speed, max_rudder_speed)
...
environment = [INITIAL_SAIL_ANGLE,
               ...
               TrueWind(INITIAL_WIND_STRENGTH * cos(radians(INITIAL_WIND_DIRECTION)),
                        ...
                        sin(radians(INITIAL_WIND_DIRECTION)),
                        INITIAL_WIND_STRENGTH,
...
def step(event):
    ...
    state = ode(solve).set_integrator(
        'dopri5', nsteps=1000).set_initial_value(state, 0).integrate(STEPSIZE)
