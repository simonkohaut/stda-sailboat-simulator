...
def scenario_1(save=False):
    ...
    plots_manoevers(t, x, r, sail, ref_heading, save, sail_force=sail_force,
                    keel_force=keel, separation=separation, wind=wind, append=append)
...
def comp_route(x, x2):
    fig_traj, ax_traj = plot_series(
        x[POS_X, :], x[POS_Y, :], xlabel='$x$ / m', ylabel='$y$ / m')
    fig_traj, ax_traj = plot_series(
        x2[POS_X, :], x2[POS_Y, :], xlabel='$x$ / m', ylabel='$y$ / m', fig=fig_traj, ax=ax_traj)
...
def plots_manoevers(t, x, r, sail, ref_heading, save, sail_force=None, keel_force=None, separation=None, wind=None, append=""):
    ...
    fig_speed, tmp = plot_time_fig(
        time=t, data=speeds, labels=speed_labels, ax_labels=speed_axlabels)
    ...
    fig_ang, tmp = plot_time_fig(
        time=t, data=angles, labels=angles_labels, ax_labels=axlabels, scale=scale)

    fig_traj, ax_traj = plot_series(
        x[POS_X, :], x[POS_Y, :], xlabel='$x_g$ / m', ylabel='$y_g$ / m')
    plot_arrows(x[POS_X, :], x[POS_Y, :], x[YAW, :],
                fig=fig_traj, ax=ax_traj, color='red')
    ax_traj.plot(x[POS_X, 0], x[POS_Y, 0], 'xb', markersize=10)
    ax_traj.arrow(0, -25, wind.x, wind.y, head_width=1.5,
                  head_length=3, width=.5, fc='green', ec='green')
    # ax_traj.legend(['route', 'start', 'wind'])

    # heading plot
    heading_data = [x[YAW, :] + 2*pi, ref_heading,
                    arctan2(x[VEL_Y, :], x[VEL_X, :])]
    heading_labels = ['Heading', 'Reference', 'Drift']
    fig_heading, ax_heading = plot_time_fig(
        time=t, data=heading_data, labels=heading_labels, ax_labels=axlabels, scale=scale, ticks='ang')
...
def plot_time_fig(time, data, labels, ax_labels, scale=1., ticks=None):
    ...
    for i, data_i in enumerate(data):
        fig, ax = plot_series(
            time, data_i*scale, label=labels[i], xlabel=ax_labels[0], ylabel=ax_labels[1], fig=fig, ax=ax, legend=True)
...
def plot_old():
    ...
    fig_vx, tmp = plot_series(
        t, x[VEL_X, :], xlabel='time /s', ylabel='speed $x$ / m/s')
    fig_vy, tmp = plot_series(
        t, x[VEL_Y, :], xlabel='time /s', ylabel='speed $y$ / m/s')
    fig_heading, ax_heading = plot_series(
        t, (x[YAW, :]/pi*180) % 360, xlabel='time /s', ylabel='heading / deg')

    drift = drift = arctan2(x[VEL_Y, :], x[VEL_X, :])
    fig_heading, ax_heading = plot_series(
        t, ref_heading/pi*180, xlabel='time /s', ylabel='heading / deg', fig=fig_heading, ax=ax_heading)
    fig_heading, ax_heading = plot_series(
        t, drift/pi*180, xlabel='time /s', ylabel='heading / deg', fig=fig_heading, ax=ax_heading)

    fig_r, ax_r = plot_series(
        t, r/pi*180, xlabel='time /s', ylabel='rudder / deg')
    if actor_dynamics:
        fig_r, ax_r = plot_series(
            t, x[12, :]/pi*180, xlabel='time /s', ylabel='rudder / deg', fig=fig_r, ax=ax_r)
    # plot_series(t, x[ROLL,:]/pi*180, xlabel='time /s', ylabel='rudder / deg', fig=fig_r, ax=ax_r)
    fig_sail, ax_sail = plot_series(
        t, sail/pi*180, xlabel='time /s', ylabel='sail angle / deg')
    if actor_dynamics:
        plot_series(t, x[13, :]/pi*180, xlabel='time /s',
                    ylabel='sail angle / deg', fig=fig_sail, ax=ax_sail)
    fig_traj, ax_traj = plot_series(
        x[POS_X, :], x[POS_Y, :], xlabel='$x_g$ / m', ylabel='$y_g$ / m')
    ...
    fig_pitch, tmp = plot_series(
        t, x[PITCH, :]/pi*180, xlabel='time /s', ylabel='pitch in deg')
    fig_roll, tmp = plot_series(
        t, x[ROLL, :]/pi*180, xlabel='time /s', ylabel='roll in deg')
...
def plot_leeways(t, separation, vx, vy):
    ax, fig = None, None
    fig, ax = plot_series(t, vx, xlabel='Time /s',
                          label='Forward speed in $m/s$', ax=ax, fig=fig)
    fig, ax = plot_series(t, vy, xlabel='Time /s',
                          label='Leeway in $m/s$', ax=ax, fig=fig)
    fig, ax = plot_series(t, separation, xlabel='Time /s',
                          label='Flow separation factor', ax=ax, fig=fig)
...
def simulate(N_steps, x, t, r, sail, environment, controller, integrator, sampletime, sail_sampletime, ref_heading, wind=None, predefined_sail_angle=None):
    ...
    for i in range(N_steps):
        ...
        environment[RUDDER_ANGLE] = controller.controll(
            ref_heading[i], x[YAW, i], x[YAW_RATE, i], speed, x[ROLL, i], drift_angle=drift)
        # if t[i] < 80:
        ...
        if not predefined_sail_angle is None:
            ...
            if not i % int(sail_sampletime/sampletime):
                apparent_wind = calculate_apparent_wind(
                    x[YAW, i], x[VEL_X, i], x[VEL_Y, i], wind)
                environment[SAIL_ANGLE] = sail_angle(
                    apparent_wind.angle, apparent_wind.speed, SAIL_STRETCHING)
        ...
        x[:, i+1] = integrator.y
        # print 'time', t[i]
    print(('computation time', datetime.utcnow()-t1))
    # forces
    ...
    for i in range(x.shape[1]):
        apparent_wind = calculate_apparent_wind(
            x[YAW, i], x[VEL_X, i], x[VEL_Y, i], wind)
        force = calculate_sail_force(
            x[ROLL, i], apparent_wind, np.sign(apparent_wind.angle)*sail[i])
        sail_force[:, i] = [force.x, force.y]
        speed = np.sqrt(x[VEL_X, i]**2 + x[VEL_Y, i]**2)
        lateral_force, lateral_separation = calculate_lateral_force(
            x[VEL_X, i], x[VEL_Y, i], x[YAW, i], speed)
...
def plot_arrows(x, y, directions, fig=None, ax=None, color=None):
    ...
    for i in range(x.shape[0]):
        ax.arrow(x[i], y[i], length*cos(directions[i]), length *
                 sin(directions[i]), head_width=.05, head_length=.1, fc=color, ec=color)
